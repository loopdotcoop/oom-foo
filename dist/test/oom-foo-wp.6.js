...
describe('B.R.E.A.D. Oom.Foo instances', function () {
    ...
    //// Connection, and oom-foo exists.
    it(urlCPT + ' is recognised', function (done) {
        this.timeout(5000)
        pingPlugin()
        function pingPlugin () {
            $.ajax(wpBase + 'wp-content/plugins/oom-foo/wp-plugin.php?ping')
               .done(function (data, textStatus, jqXHR) {
                    eq(data, 'pong'
                      , 'The oom-foo plugin should ‘pong’ when you ping it')
                    if ('pong' === data)
                        recogniseCpt()
                })
    ...
    it(`Create an ${nameCPT} via WP API, ${username}:${password}` , function (done) {
        this.timeout(5000)
        const title = 'A cyan Oom.Foo, unit test random = ' + Math.random()
...
})//describe('B.R.E.A.D. Oom.Foo instances')
...
describe('B.R.E.A.D. Oom.Foo.Post instances', function () {
    ...
    //// Connection, and oom-foo-post exists.
    it(urlCPT + ' is recognised', function (done) {
        this.timeout(5000)
        pingPlugin()
        function pingPlugin () {
    ...
    it(`Create an ${nameCPT} via WP API, ${username}:${password}` , function (done) {
        this.timeout(5000)
        const title = 'A cyan Oom.Foo.Post, unit test random = ' + Math.random()
        const goodAuth = window.btoa(username + ":" + password)
        const goodSettings = {
            contentType: 'application/json'
          , method: 'POST'
...
})//describe('B.R.E.A.D. Oom.Foo.Post instances')
...
describe('B.R.E.A.D. Oom.Foo.Router instances', function () {
    ...
    //// Connection, and oom-foo-router exists.
    it(urlCPT + ' is recognised', function (done) {
        this.timeout(5000)
        pingPlugin()
        function pingPlugin () {
    ...
    it(`Create an ${nameCPT} via WP API, ${username}:${password}` , function (done) {
        this.timeout(5000)
        const title = 'A cyan Oom.Foo.Router, unit test random = ' + Math.random()
        const goodAuth = window.btoa(username + ":" + password)
        const goodSettings = {
            contentType: 'application/json'
          , method: 'POST'
...
})//describe('B.R.E.A.D. Oom.Foo.Router instances')
