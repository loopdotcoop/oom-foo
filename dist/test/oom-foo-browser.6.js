...
describe('The Oom.Foo.devThumbAFrame*() set', function (done) {
    ...
    //// Oom.Foo.devThumbAFrameVue(): `hilite` static and attribute - change.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('changing `stat/attr.hilite` changes box color', function (done) {
        const { firstObj, firstHex, secondObj, secondHex } = generateRandomColors()
        stat.hilite = firstHex
    ...
    //// Oom.Foo.devThumbAFrameVue(): boxes respond to click.
    it('boxes can change `stat/attr.hilite` on click', function (done) {
        const { thirdObj, thirdHex, fourthObj, fourthHex } = generateRandomColors()
        const onOomEvent = function (evt) {
            if (! evt.detail) return
...
})//describe('The Oom.Foo.devThumbAFrameVue() component')
...
describe('The Oom.Foo.Post.devThumbAFrame*() set', function (done) {
    ...
    //// Oom.Foo.Post.devThumbAFrameVue(): `hilite` static and attribute - change.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('changing `stat/attr.hilite` changes box color', function (done) {
        const { firstObj, firstHex, secondObj, secondHex } = generateRandomColors()
        stat.hilite = firstHex
    ...
    //// Oom.Foo.Post.devThumbAFrameVue(): boxes respond to click.
    it('boxes can change `stat/attr.hilite` on click', function (done) {
        const { thirdObj, thirdHex, fourthObj, fourthHex } = generateRandomColors()
        const onOomEvent = function (evt) {
            if (! evt.detail) return
...
})//describe('The Oom.Foo.Post.devThumbAFrameVue() component')
...
describe('The Oom.Foo.Router.devMainVue() component', function (done) {
    ...
    //// Oom.Foo.Router.devMainVue(): Automatic statics - initial values.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('shows correct initial statics', function (done) {
        Vue.nextTick((function(){let error;try{
            for (let key in stat) {
    ...
    //// Oom.Foo.Router.devMainVue(): Automatic read-only statics - shows changes.
    it('shows that read-only statics have changed', function (done) {
        const cache = { good:{} }
        for (let key in stat) {
            if (! isReadOnly(key) ) continue // only read-only properties
    ...
    //// Oom.Foo.Router.devMainVue(): Automatic read-write statics - shows changes.
    it('shows that read-write statics have changed', function (done) {
        const cache = { good:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue // only read-write properties
    ...
    //// Oom.Foo.Router.devMainVue(): Automatic read-write statics - valid input.
    it('updates read-write statics after UI input', function (done) {
        const cache = { $el:{}, good:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue
    ...
    //// Oom.Foo.Router.devMainVue(): Automatic read-write statics - invalid input.
    it('does not update read-write statics after invalid UI input', function (done) {
        const cache = { $el:{}, orig:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue
...
describe('The Oom.Foo.Router.devThumbAFrame*() set', function (done) {
    ...
    //// Oom.Foo.Router.devThumbAFrameVue(): `hilite` static and attribute - change.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('changing `stat/attr.hilite` changes box color', function (done) {
        const { firstObj, firstHex, secondObj, secondHex } = generateRandomColors()
        stat.hilite = firstHex
    ...
    //// Oom.Foo.Router.devThumbAFrameVue(): boxes respond to click.
    it('boxes can change `stat/attr.hilite` on click', function (done) {
        const { thirdObj, thirdHex, fourthObj, fourthHex } = generateRandomColors()
        const onOomEvent = function (evt) {
            if (! evt.detail) return
...
})//describe('The Oom.Foo.Router.devThumbAFrameVue() component')
